...
Page({
  ...
  chooseReply(e) {
    ...
    const { replyNickname } = that.data;
    const { replyNameid } = that.data;
  ...
  loadNew(e) {
    const self = this;
    const imgsLen = jsonLength(self.data.catSwiperImgs);
      ...
      wx.request({
        ...
        success(res) {
          if (res.data) {
            ...
            const catImgs = self.data.catImgs.concat(res.data);
  ...
  chooseImg() {
    wx.chooseImage({
      success: (res) => {
        ...
        const len1 = temp.length;
        const len2 = this.data.imgs.length;
        if (len1 + len2 > 9) {
          wx.showToast({
            icon: 'none',
            title: '最多选九张照片！',
